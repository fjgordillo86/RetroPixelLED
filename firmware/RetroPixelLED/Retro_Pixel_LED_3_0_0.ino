...
//                          CONSTANTES & FIRMWARE
// ====================================================================
#define FIRMWARE_VERSION "3.0.0" // MODIFICADO: Nueva versión del firmware que implemeta listado de GIFs sin limite listanod directamente en la SD sin pasar por RAM.
...
String webPage() {
    ...
    html += "<option value='0'" + String(config.clockEffect == 0 ? " selected" : "") + ">Rainbow Flow</option>";
    html += "<option value='1'" + String(config.clockEffect == 1 ? " selected" : "") + ">Static Rainbow</option>";
    html += "<option value='2'" + String(config.clockEffect == 2 ? " selected" : "") + ">Solid Neon</option>";
    html += "<option value='3'" + String(config.clockEffect == 3 ? " selected" : "") + ">Night Fire</option>";
    html += "<option value='4'" + String(config.clockEffect == 4 ? " selected" : "") + ">Pulse Breath</option>";
    html += "<option value='5'" + String(config.clockEffect == 5 ? " selected" : "") + ">Matrix Digital</option>";
    html += "<option value='6'" + String(config.clockEffect == 6 ? " selected" : "") + ">Color Gradient 50%</option>";
    html += "<option value='7'" + String(config.clockEffect == 7 ? " selected" : "") + ">Color Gradient 80%</option>";
    html += "  </select></div>";
    ...
    html += "  <a href='/file_manager' class='btn btn-files' onclick='return confirm(\"Se detendrá el panel para gestionar archivos. ¿Continuar?\")'>FILES</a>";
    ...
    html += "<script>"
            ...
            "  document.getElementById('clockConfig').style.display = (m=='2')?'block':'none';"
            "};"
...
String configPage() {
    ...
    // Modo Funcionamiento Wifi
    html += "<label>Modo de Funcionamiento</label>";
    html += "<select name='wifiOffMode'>";
    ...
    html += "Al cambiar de modo, <b>debes Guardar y Reiniciar</b>. ";
    html += "<br><i>* Si no se configura una red WiFi en 3 min, se activa el Modo Offline automáticamente.</i>";
    html += "</div><br>";

    // Zona Horaria TZ
    html += "<label>Zona Horaria (TZ String)</label><input type='text' name='tz' value='" + config.timeZone + "'>";
    html += "<div class='info-box'>Valor por defecto para España: <code>" + String(TZ_STRING_SPAIN) + "</code>. <br>Puedes buscar otras zonas horarias aquí: <a href='https://github.com/nayarsystems/posix_tz_db/blob/master/zones.csv' target='_blank' style='color:#00fbff;'>Lista de TZ Strings</a>. La gestión del DST es automática.</div>";
    html += "</div>";

    // 2. HARDWARE
    html += "<div class='card'><h2>2. Hardware & Panel</h2>"; 
    // Paneles en cadena
    html += "<label>Número de Paneles (Chain)</label>";
    html += "<input type='number' name='pc' min='1' max='8' value='" + String(config.panelChain) + "'>";

    html += "<div class='grid-2'>"; 
    // 2.1. Velocidad I2S
    html += "<div><label>Velocidad I2S</label><select name='i2s'>";
    html += "<option value='0'" + String(config.i2sSpeed == 0 ? " selected" : "") + ">8 MHz (Seguro)</option>";
    html += "<option value='1'" + String(config.i2sSpeed == 1 ? " selected" : "") + ">10 MHz (Estable)</option>";
    html += "<option value='2'" + String(config.i2sSpeed == 2 ? " selected" : "") + ">16 MHz (Normal)</option>";
    html += "<option value='3'" + String(config.i2sSpeed == 3 ? " selected" : "") + ">20 MHz (Turbo)</option>";
    html += "</select></div>";

    // 2.2. Min Refresh Rate
    html += "<div><label>Refresco Mín. (Hz)</label>";
    html += "<input type='number' name='mrr' min='30' max='120' value='" + String(config.minRefreshRate) + "'></div>";
    html += "</div>";

    // 2.3. Latch Blanking (Ghosting)
    html += "<label>Latch Blanking (Anti-Ghosting)</label>";
    ...
    html += "<span id='lbVal' style='color:#00f2ff; font-weight:bold; font-size:14px; width:20px;'>" + String(config.latchBlanking) + "</span>";
    html += "</div>";
    html += "<div class='info-box'>⚠️ <b>Nota:</b> Subir el 'Latch Blanking' reduce el brillo fantasma.un refesco muy alto puede desiquilibar la conexión WiFi. <br>Cambiar estos valores requiere <b>Guardar y Reiniciar</b>.</div>";
    html += "</div>";
...
String obtenerSiguienteGifSD() {
    ...
    // 1. Saltamos a la posición guardada
    if (gifCachePosition > 0) {
        cacheFile.seek(gifCachePosition);
    }

    // 2. Si hemos llegado al final del archivo, volvemos al principio (Loop)
    if (!cacheFile.available()) {
        cacheFile.seek(0);
        gifCachePosition = 0;
    }

    // 3. Leemos la línea (la ruta del GIF)
    String gifPath = cacheFile.readStringUntil('\n');
    gifPath.trim();

    // 4. Guardamos la posición actual para la próxima vez
...
void setup() {
    ...
    // --- 0. OBTENER ID ÚNICO (VERSIÓN UNIVERSAL CORE 3.X) ---
    ...
    // --- APLICACIÓN DE AJUSTES AVANZADOS  ---
...
void TaskDisplay(void * pvParameters) {
    for (;;) {
        ...
        // 2. Si el panel está apagado por software
        ...
        // 3. Si estamos en el Gestor de Archivos (enModoGestion == true)
        ...
        // 4. Ejecución Normal con Semáforo (Si NO estamos en gestión)
