...
//                          CONSTANTES & FIRMWARE
// ====================================================================
#define FIRMWARE_VERSION "4.0.1" // Correción de errores
...
HUB75_I2S_CFG matrix_config; // Se rellena en setup(); global para poder recrear 'display' desde loop() (ver gestión del portal WiFi)
...
struct Config {
    ...
    uint32_t slidingTextColor = 0xFFFF;
...
void loadConfig() { 
    ...
    config.slidingTextColor = preferences.getULong("slideColor", 0xFFFF);
...
void handleOTAUpload() {
    ...
    if (upload.status == UPLOAD_FILE_START) {
        ...
        // IMPORTANTE: TaskDisplay puede estar en ese instante dentro de un
        // xSemaphoreTake(sdMutex, ...). Si la matamos a lo bruto con
        // vTaskDelete() mientras tiene el mutex tomado, el mutex queda
        // "huérfano" para siempre y cualquier otro xSemaphoreTake(sdMutex,...)
        // posterior (incluida esta misma subida OTA al tocar la SD) se
        // queda colgado sin remedio. Por eso primero intentamos tomar
        // nosotros el mutex: si lo conseguimos, es que TaskDisplay no está
        // en mitad de una sección crítica y es seguro eliminarla.
        if (displayTaskHandle != NULL) {
            if (xSemaphoreTake(sdMutex, pdMS_TO_TICKS(2000))) {
                ...
                // TaskDisplay lleva más de 2s sin soltar el mutex (situación
                // anómala). La eliminamos igualmente para no bloquear la OTA,
                // pero recreamos el semáforo desde cero para no arrastrar un
                // mutex huérfano el resto de la sesión.
                Serial.println("OTA: sdMutex no disponible tras 2s, recreando semáforo antes de continuar.");
                vTaskDelete(displayTaskHandle);
                displayTaskHandle = NULL;
...
void ejecutarModoReloj() {
    ...
    static unsigned long ultimoRedibujado = 0;

    const unsigned long INTERVALO_REDIBUJADO_MS = 33; // ~30 fps
    unsigned long ahora = millis();
    bool cambioEstiloOModo = (modoAnterior != config.playMode) || (estiloAnterior != config.clockEffect);

    if (!cambioEstiloOModo && (ahora - ultimoRedibujado < INTERVALO_REDIBUJADO_MS)) {
        return;
    }
...
// Estado del reintento no bloqueante de MQTT (antes: while()+vTaskDelay que
// congelaba loop() -y con él server.handleClient()- hasta 25s de golpe)
static int mqttReintentos = 0;
static unsigned long mqttUltimoIntento = 0;
const int MQTT_MAX_REINTENTOS = 5;
const unsigned long MQTT_INTERVALO_REINTENTO_MS = 5000;
...
void reconnectMQTT() {
    ...
    // Solo probamos una vez cada MQTT_INTERVALO_REINTENTO_MS, y devolvemos
    // el control a loop() inmediatamente en el resto de llamadas: así el
    // servidor web sigue respondiendo mientras el broker está caído.
    unsigned long ahora = millis();
    if (ahora - mqttUltimoIntento < MQTT_INTERVALO_REINTENTO_MS) return;
    mqttUltimoIntento = ahora;

    if (mqttReintentos >= MQTT_MAX_REINTENTOS) {
        // Nos rendimos por ahora; el propio contador de tiempo de arriba
        // hace que se vuelva a intentar (a ritmo lento) en el próximo ciclo.
        return;
    }

    String technicalID = "retropixel_" + chipID;
    Serial.printf("Intentando conexión MQTT (Intento %d/%d)...\n", mqttReintentos + 1, MQTT_MAX_REINTENTOS);
    ...
    if (mqttClient.connect(technicalID.c_str(), config.mqtt_user, config.mqtt_pass)) {
        ...
        mqttReintentos = 0; // Reseteamos contador al conectar
    } else {
        ...
        mqttReintentos++;
        if (mqttReintentos >= MQTT_MAX_REINTENTOS) {
            Serial.println("MQTT: Máximo de reintentos alcanzado. Se reintentará más despacio en próximos ciclos.");
        }
    }
}
...
void setup() {
    ...
    // Guardamos aparte el texto personalizado del usuario (ya cargado desde
    // Preferences por loadConfig()) ANTES de que el bloque de conexión WiFi
    // lo sobreescriba con mensajes de estado. Así podemos restaurarlo en
    // cuanto la conexión (o el modo offline) quede resuelta, en vez de
    // perderlo hasta que el usuario lo vuelva a escribir a mano.
    String textoPersonalizadoGuardado = config.slidingText;
    ...
    wm.setConfigPortalTimeout(180); // 3 minutos de espera, luego el portal se cierra solo

    bool tieneTextoPersonalizado = !textoPersonalizadoGuardado.startsWith("Retro Pixel LED v");

    wm.setSaveConfigCallback([tieneTextoPersonalizado, textoPersonalizadoGuardado]() {
        Serial.println(">> WiFiManager: credenciales guardadas y conectado con éxito.");
    ...
    if (config.WifiOffMode) {
        ...
        config.slidingText = tieneTextoPersonalizado
            ? textoPersonalizadoGuardado
            : "Modo Offline Activo - Conectate a la IP: 192.168.4.1 para usar Retro Pixel LED";
    } else {
        // ESCENARIO B: Intento Online
        config.slidingText = "MODO ONLINE - Buscando WiFi...";

        // Intentamos conectar (no bloqueante: si no hay credenciales guardadas
        // o fallan, esto lanza el portal en segundo plano y vuelve aquí de inmediato)
        if (!wm.autoConnect("Retro Pixel LED")) {
            ...
            // Si conecta a la primera (con credenciales ya guardadas):
            Serial.println("Conectado con éxito.");
            config.slidingText = tieneTextoPersonalizado
                ? textoPersonalizadoGuardado
                : "Retro Pixel LED v" + String(FIRMWARE_VERSION) + " - IP: " + WiFi.localIP().toString();
        }
    }
    ...
    if (display) {
        // --- SISTEMA ANTI-PANIC (portado de Retro Pixel LED Lite) ---
        displayOk = display->begin();
        if (!displayOk) {
            Serial.println("[ANTI-PANIC] Fallo al iniciar el display con Double Buffer (heap fragmentado). Reintentando en modo Single Buffer...");
            delete display;
            matrix_config.double_buff = false;
            display = new MatrixPanel_I2S_DMA(matrix_config);
            displayOk = display ? display->begin() : false;
            if (displayOk) {
                Serial.println("[ANTI-PANIC] Recuperado: display iniciado en Single Buffer.");
            } else {
                Serial.println("[ANTI-PANIC] ERROR CRÍTICO: el display no arrancó ni siquiera en Single Buffer.");
            }
        }
    }

    if (display && displayOk) {
...
bool displayPausadoPorPortal = false;

void loop() {
    // 1. Si el portal de WiFiManager está funcionando
    if (wm.getConfigPortalActive()) {

        if (display && !displayPausadoPorPortal) {
            ...
            const char* linea1 = "Conectate al AP";
            const char* linea2 = "y elige tu WiFi";
            display->getTextBounds(linea1, 0, 0, &bx, &by, &bw, &bh);
            display->setCursor(128 + 19, (MATRIX_HEIGHT / 2) - (int)bh - 1);
            display->print(linea1);
            display->getTextBounds(linea2, 0, 0, &bx, &by, &bw, &bh);
            display->setCursor(128 + 19, (MATRIX_HEIGHT / 2) + 1);
            ...
            delete display;
            display = nullptr;
            displayPausadoPorPortal = true;
            Serial.println(">> Portal WiFi activo: panel apagado para no interferir con el escaneo de redes.");
        }
        wm.process();
        modoTemporalActivado = false; // Resetear bandera si el portal está activo
    } 
    else {
        if (displayPausadoPorPortal) {
            ...
            displayPausadoPorPortal = false;
            Serial.println(">> Portal WiFi cerrado: panel reactivado.");
        }
